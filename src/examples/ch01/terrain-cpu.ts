...
import { createTerrainData } from '../../common/terrain-data';
...
const run = async () => {
    ...
    folder.add(params, 'seed', 1, 65536, 1).onChange(()=>{
        dataChanged = true;
    }); 
    folder.add(params, 'scale', 0.3, 100, 0.1).onChange(()=>{
        dataChanged = true;
    }); 
    folder.add(params, 'xWidth', 40, 500, 1).onChange((val)=>{
        dataChanged = true;
    }); 
    folder.add(params, 'zHeight', 40, 500, 1).onChange((val)=>{
        dataChanged = true;
    }); 
    folder.add(params, 'aspectRatio', 0, 100, 0.1).onChange(()=>{
        dataChanged = true;
    }); 
    folder.add(params, 'octaves', 1, 20, 1).onChange(()=>{
        dataChanged = true;
    }); 
    folder.add(params, 'persistence', 0, 1, 0.01).onChange(()=>{
        dataChanged = true;
    }); 
    folder.add(params, 'lacunarity', 1, 10, 0.2).onChange(()=>{
        dataChanged = true;
    }); 
    folder.add(params, 'offsetX', 0, 100, 0.2).onChange(()=>{
        dataChanged = true;
    }); 
    folder.add(params, 'offsetZ', 0, 100, 0.2).onChange(()=>{
        dataChanged = true;
    }); 
    folder.add(params, 'colormap', [
        'terrain', 'ocean', 'autumn', 'bone', 'cool', 'copper', 'greys', 'hsv', 'hot', 
        'jet', 'ocean', 'rainbow', 'rainbow_soft', 'spring', 'summer', 'winter'
    ]).onChange(()=>{
        dataChanged = true;
    });  
    folder.add(params, 'wireframeColor', [
        'white', 'black', 'red', 'green', 'blue', 'yellow', 'cyan', 'fuchsia',
        'terrain', 'ocean', 'autumn', 'bone', 'cool', 'copper', 'greys', 'hsv', 'hot', 
        'jet', 'ocean', 'rainbow', 'rainbow_soft', 'spring', 'summer', 'winter'
    ]).onChange(()=>{
        dataChanged = true;
    ...
    let modelMat = ws.createModelMat([-0.65*params.xWidth, 5, -0.5*params.zHeight], 
    ...
    const frame = () => {     
        ...
        if(dataChanged){
            ...
            modelMat = ws.createModelMat([-0.65*params.xWidth, 5, -0.5*params.zHeight], 
                [0, Math.PI/15, 0], [1, params.aspectRatio, 1]);
            mvpMat = ws.combineMvpMat(modelMat, viewMat, projectMat);
            init.device.queue.writeBuffer(p.uniformBuffers[0], 0, mvpMat as ArrayBuffer);
